...
export class PesquisasService extends RequestService {
    constructor(_httpClient: HttpClient, private _http: Http) {
        super(_httpClient);
    }
    ...
    toResultadoModel(resultado: {
        id: number;
        res: { localidade: string; res: any }[];
    }) {
        let valorMaisRecente = Object.keys(resultado.res[0].res).reduce(
            (agg, key) => {
    ...
    private _getPesquisa(pesquisaId = '') {
        return this.request(
            `http://servicodados.ibge.gov.br/api/v1/pesquisas/${pesquisaId}`
        );
    }
    ...
    private _getMetadata(pesquisaId: string, indicadorId: string) {
        ...
        return this.request(
            `http://servicodados.ibge.gov.br/api/v1/pesquisas/${pesquisaId}/indicadores/${indicadorId}`
        ).map(metadata =>
            this.flatMetadata(metadata).map(this.toMetadataModel)
        );
    }

    private _getResultado(
        pesquisaId: string,
        indicadorId: string,
        localidadeId: string
    ) {
        ...
        return this.request(
            `http://servicodados.ibge.gov.br/api/v1/pesquisas/${pesquisaId}/indicadores/${indicadorId}/resultados/${localidadeId}`
