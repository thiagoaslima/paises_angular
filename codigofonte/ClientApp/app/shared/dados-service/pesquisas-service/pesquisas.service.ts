...
import { PesquisaResposta, PesquisaMetadataResultado, PesquisaResultado, Pesquisa } from './pesquisas.interface';
...
export class PesquisasService {
    ...
    toPesquisaModel(pesquisa: any): Pesquisa {
        return new Pesquisa(pesquisa)
    }

    toMetadataModel(metadata: any): PesquisaMetadataResultado {
        return {
            id: metadata.id,
    ...
    toResultadoModel(resultado: { id: number, res: { localidade: string, res: any }[] }): PesquisaResultado {
    ...
    get(identificador: { pesquisaId: string, indicadorId: string, localidadeId: string }): Observable<PesquisaResposta> {
