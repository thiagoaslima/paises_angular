...
export function RxMultiCache({
  cache,
  labelsFromArguments,
  labelsFromResponse
}: {
  cache: BasicLRUCache;
  labelsFromArguments: (...args: any[]) => Array<number | string>;
  labelsFromResponse: (...args: any[]) => Array<number | string>;
}) {
  return function _RxMultiCache(
    target: any,
    propertyKey: string,
    ...
    descriptor.value = function(...args: any[]) {
      ...
      let { caches, notFound } = items.reduce(
        (acc, label) => {
          let item = cache.get(label);

          if (!item) {
            item = new ReplaySubject<any>(1);
